...
name: reviewer
description: |
  Use for /reviewer, review, readability review, QA, security review, or design review requests.
  Route to fresh-context readability, isolated reproduction QA, and security agents as requested.
  Manual invocation only; QA may create local reproduction artifacts but may not fix product code.
---
...
가독성·보안은 읽기 전용이며, QA만 독립 작업 공간에서 재현용 파일을 만들고 실행할 수 있다.
...
| [qa](../../agents/qa.md) | 기획과 경계 사례를 재현했을 때 동작·비용·성능·지연에 문제가 있는가 |
| [security-reviewer](../../agents/security-reviewer.md) | 입력·권한·의존성에서 악용 가능한 문제가 있는가 |

readability는 코드의 이해 비용, QA는 관찰 가능한 동작과 측정, security는 악용 가능성을 담당한다.
예를 들어 반복 요청은 QA가 결과·호출 비용을 재현하고, 권한 우회 가능성은 security가 검토한다.
...
이는 프로젝트 기록에 대한 제한이다. QA의 임시 재현 파일은 해당 독립 공간에만 둘 수 있다.
...
일반 리뷰 요청은 readability·QA·security를 새 context로 병렬 실행한다. 가독성·QA·보안만
요청했다면 해당 역할만 실행한다. 설계 검토는 readability가 맡고, 실행 가능한 구현이 없으면
QA 재현을 한 것으로 취급하지 않는다. 구현자·이전 리뷰어를 resume하지 않는다.

QA에는 대상 코드의 원본 경로·커밋·미커밋 변경 범위와 요구·실행 환경·허용 한도를 전달한다.
QA의 [격리·재현 지침](../../agents/qa.md)에 따라 독립 작업 공간을 확보하게 한다.
도구가 만든 worktree도 대상과 같은 내용인지 확인해야 한다. 자동 worktree 생성만으로 미커밋 변경이
포함되었다고 가정하지 않는다. 격리가 안 되면 QA는 미완료로 보고하며 원본 공간에서 대신 실행하지 않는다.

```text
Agent(subagent_type="lstack:readability-reviewer", prompt="<요구·경로·변경·적용 기준>. 사람이 읽는 비용 확인.")
Agent(subagent_type="lstack:qa", isolation="worktree", prompt="<요구·원본 경로·대상 커밋·미커밋 범위·환경·한도>. 대상과 일치하는 독립 공간에서 사례 재현.")
...
QA는 기대·실제 결과와 재현 조건·측정 근거를 보고한다. 재현 불가를 통과로 바꾸지 않으며,
