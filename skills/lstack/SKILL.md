...
대상 plan.md 의 섹션 상태로 phase 를 추론한다. SSOT 는 `docs/spec/ARCHITECTURE.md` §
"plan.md 섹션 → Phase 매핑" 표. 요약:

- `## 배경` / `## 설계` / `### 결정` / `### Codex 검토` / `### 최종 확정` / `## 태스크` / AC / 상태마커 순으로 섹션 존재 여부를 확인 → 해당 phase 로 분기.
- 승인 마커 (`### 최종 확정 (User 승인 YYYY-MM-DD)`) 부재 시 Phase 2.3 (승인 대기) 로 판정.
...
## 설계
(principal-engineer (Claude) 가 `### 결정` · `### 리스크` 작성;
 principal-engineer (Codex) 가 `### Codex 검토` append; user 승인 시 `### 최종 확정` 추가)

## 태스크
(planner 가 ### Tn: skeleton, test-planner 가 AC 추가)

## 향후 과제
```

### 2.1-2.3: `call-codex-cli(lstack:principal-engineer)` (조사 + 설계, SSOT)
...
Skill({
  ...
  args:
    ...
    context: |
      ...
      plan.md ## 배경을 읽고 코드베이스를 조사(정적 구조 + 런타임 동작 시퀀스)한 뒤
      ## 설계 섹션을 작성하고 <memo for="planner"> 반환.
})
```

Codex가 로컬 코드를 직접 읽어 조사 + 설계. `write=true` 로 plan.md에 직접 기록.
Codex 미설치 시 skill이 hard fail — 에러를 메인 컨텍스트에 그대로 보고, PM이 사용자 에스컬레이션 판단.
...
Agent({
  ...
  prompt: <포함>
    ...
    - "## 태스크 아래에 ### Tn: 헤더로 태스크 skeleton 작성 (태스크 제목 + exec agent + 1-3줄 구현 힌트).
...
Agent({
  ...
  prompt: <포함>
    ...
    - "plan.md 를 읽고 ## 태스크 의 각 ### Tn 블록 끝에 AC 체크박스를 추가하세요.
...
| 복잡성 정리 (자동) | `call-codex-cli(lstack:principal-engineer)` mode: refactor | review 가 복잡성 신호 보고 시 동작 보존 리팩터. orchestrator 가 자동 호출 — task 로 직접 박지 말 것 |
