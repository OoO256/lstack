...
| `## 배경`만 있고 `## 설계` 비어있음 | **Phase 2.1-2.3** (architect → codex-architect) |
...
### 2.1-2.3: codex-architect (조사 + 설계, SSOT)
...
Agent({
  ...
  prompt: <포함>
    - plan.md 경로
    - "코드베이스를 조사하고 (정적 구조 + 런타임 동작 시퀀스), ## 설계를 작성하세요.
       Codex 가용 시 design을 Codex에 위임. 미가용 시 자체 설계."
})
```

codex-architect가 Phase 2.1 (investigation)을 직접 수행 후, Codex availability에 따라
design을 Codex에 위임하거나 자체 수행. `DESIGN_BACKEND` 로 어느 경로 갔는지 보고.
...
| 복잡성 정리 (자동) | `lstack:codex-architect` (mode: review) | code review 복잡성 신호에 패턴 카탈로그 적용. orchestrator가 자동 호출 — task로 직접 박지 말 것 |
