...
| `## 배경`만 있고 `## 설계` 비어있음 | **Phase 2.1-2.3** (`call-codex-cli(lstack:principal-engineer)`) |
| `## 설계` 작성됐고 `## 태스크` 비어있음 | **Phase 2.4** (planner) — 태스크 skeleton |
| `## 태스크`에 `### Tn:` 있고 AC 비어있음 | **Phase 2.5** (test-planner) → 사용자 승인 |
| `## 태스크`에 상태 마커 없는(대기) `### Tn:` 있음 | **Phase 3+4** (orchestrator) |
| 모든 `### Tn:`이 `— 완료`이며 spec 변경 미확인 | **Phase 5** (Spec 업데이트) |
...
| 복잡성 정리 (자동) | `call-codex-cli(lstack:principal-engineer)` mode: review | code review 복잡성 신호에 패턴 카탈로그 적용. orchestrator가 자동 호출 — task로 직접 박지 말 것 |
