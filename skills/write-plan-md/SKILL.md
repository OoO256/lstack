...
**글쓰기 3원칙** (plan.md 모든 섹션 · close 단계 PR body 공통 SSOT):
- **구현 방침 중심.** "왜 / 어떤 결정" 위주. 파일·함수 변경 나열 X (필요하면 한두 줄로 압축).
- **외부 개발자 가독성.** 이 작업을 하지 않은 동료가 처음 읽고 이해 가능하게. 사내 약어 / 세션 컨텍스트 / "아까 그거" 식 지시 금지.
- **간결.** 짧은 bullet, 결과 중심. 프로세스(시도-실패-재시도) 기록 X.
...
- 완료 결과: 1-2줄 + 선택적 의사결정/남은 리스크. 검증 방법·코드 리뷰 로그는 기록하지 않는다.
...
plan.md 는 작업 단계에 따라 두 가지 모드로 존재한다.

| 단계 | 모드 | 독자 | 구조 |
|------|------|------|------|
| Phase 1-6 (작업 중) | **저널** | 작업자 / 오케스트레이터 | "작업 중 구조" — 설계 · Codex 검토 · 최종 확정 · agent 지침 · 시행착오 모두 기록 |
| Phase 7 (close 후) | **PR** | 외부 리뷰어 | "close 후 구조" — 중간 기록 삭제 · `## 설계` → `## 구현 원칙` 변환 |

작업 중에는 정보를 모두 기록하고 (저널), close 단계에서 한 번에 PR 모드로 변환한다 (close skill 단일 SOT). close 후 plan.md 는 resume 대상이 아니다 (작업 종료).
...
처음 보는 동료가 이 단락만 읽고 맥락을 잡을 수 있어야 한다.
...
5. **완료 시 기록 (결과 중심):**
   - 헤더에 `— 완료 \`sha\`` 추가
   - 결과 요약 1-2줄 — **방침 단어**로 (뭘 어떻게 했는지가 아니라 "왜/어떤 방침"). 계획대로면 생략 가능.
...
9. **외부 가독성 체크.** 작성 후 "이 작업 안 한 동료가 읽고 이해 가능한가?" 자문. 사내 약어, 세션 컨텍스트, "그/저/이거" 식 지시는 풀어쓴다.

## close 후 구조 (PR 모드)

Phase 7 close 에서 저널 → PR 모드로 한 번 변환한다. PR 리뷰어가 본 PR 의 **큰 원칙 + 변경 결과** 를 한눈에 보게 한다.

```markdown
# <goal>

## 배경
2-3 문장. PR 리뷰어가 1분 안에 맥락을 잡을 수 있게.

## 구현 원칙
본 PR 에서 적용한 큰 원칙 3-5 개. 파일/함수 나열 X.
- **원칙명** — 한 줄 설명. (이 결정의 이유)
- **두 번째 원칙** — …
...
## 세션
선택 섹션. 히스토리/수동 resume 힌트 용도로 보존하고 PR body 에는 복사하지 않는다.
```

### 변환 규칙 (close skill 이 수행)

**삭제하는 것:**
- `## 설계` 전체 (`### 결정`, `### 리스크`, `### Codex 검토`, `### 최종 확정` 모두) — 핵심은 `## 구현 원칙` 으로 흡수.
- `## AS-IS → TO-BE` (선택 섹션) — 작업 종료 후 무의미.
- 태스크 헤더의 `(exec: agent-name)` 마커 — 리뷰어한텐 노이즈.
- AC 행의 `(v: agent-name)` 마커 — 리뷰어한텐 노이즈.
- 태스크 본문의 `**의사결정**` / `**남은 리스크**` 라벨 — 핵심은 `## 구현 원칙` / `## 향후 과제` 로 흡수.
- `— 진행중` suffix (남아 있으면 안 됨, 모든 태스크는 `— 완료`).
- 시행착오 / retry 흔적 (이미 거의 없지만 잔존 시 제거).
...
### close 후 글쓰기 체크
- 글쓰기 3원칙 (방침 중심 / 외부 가독성 / 간결) 을 모든 섹션에 다시 적용.
- "이 PR 을 처음 보는 리뷰어가 `## 구현 원칙` 만 읽고 본질을 잡을 수 있는가?" 자문.
...
- ❌ "아까 그거", "위에서 말한 X" 식 세션 컨텍스트 의존 (→ 풀어쓴다)
- ❌ 결과 요약에 파일·함수 나열만 적기 (→ 방침/이유로 대체)
