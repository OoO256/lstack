...
│   └── start/references/          # start가 소비 환경에서 읽는 동봉 공통 원칙
├── scripts/                       # 원본 spec과 동봉 리소스의 동기화·검사
...
| 스킬 | 경로 | 역할 |
|------|------|------|
| `start` | `skills/start/SKILL.md` | 진입점. origin/main → worktree 새 브랜치 + 의도 인터뷰 + 동봉 공통 가이드 로드 + 구조 판단(의도 8) + plan.md 착수. resume 자동 판별. 프로젝트 기본값 `skills/start/projects/<basename>.md` |
...
`docs/spec/PRINCIPLE.md`가 사람이 관리하는 공통 원칙의 SSOT이고,
`skills/start/references/PRINCIPLE.md`는 `start`가 소비 환경에서 읽는 동봉 사본이다.
원본 변경 뒤 `npm run sync:start-principle`로 사본을 갱신하고,
`npm run check:start-principle`로 일치를 검증한다. 대상 프로젝트의
`docs/spec/PRINCIPLE.md`는 선택적인 추가 원칙이며, 충돌할 경우 프로젝트 원칙이 우선한다.
