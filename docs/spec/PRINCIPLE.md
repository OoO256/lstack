...
### 1.3 call-as-codex 사용
1.1, 1.2 원칙을 달성하기 위해서 call-as-codex 를 사용하여 codex agent를 spawn 할 수 있다.
...
단일 `plan.md` 파일(`docs/worklogs/YYYY-MM-DD-<confirmed_slug>/plan.md`, `confirmed_slug` 는 Phase 0.4 setup 반환값)을 통해 모든 agent가 작업 context를 공유한다.
설계, 태스크, AC, 실행 결과, 향후 과제가 모두 plan.md에 기록되며, 이것이 유일한 SOT이다.
...
- frontmatter 없음 → invokable subagent로 등록되지 않고 `call-as-codex(lstack:principal-engineer)` 로만 호출
