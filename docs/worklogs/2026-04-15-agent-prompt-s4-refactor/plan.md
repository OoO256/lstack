...
**다음 사이클 (연기된 결정)**:
- D1: orchestrator Phase 0-6 PM 승격 — 이번 사이클에서 문서 SSOT + review 계약 정렬된 뒤이므로 이제 안전하게 진행 가능. 별도 worklog.
- D4: `call-codex-cli` → `call-as-codex` rename (alias 경로).
- D5: `test-planner` → `test-designer` rename (필요성 재평가).
