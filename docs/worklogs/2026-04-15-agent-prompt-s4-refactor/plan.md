...
## 2.0.1 확장 사이클 — D1 / D4 / D5 실행

v2.0.0 배포 후 `## 향후 과제` 에 연기 기록된 D1/D4/D5 를 이 worklog 연장으로 실행한다.
설계 재검토는 이전 `### 최종 확정` 블록의 "다음 사이클" 항목에 이미 결정됨 — 원안 유지 + Codex 지적 일부 수용 (approval contract SSOT · version SSOT drift · old→new CHANGELOG 매핑 · smoke 3종).

### 확장 결정 (User 승인 2026-04-15)

- **D1 원안 유지**: `agents/orchestrator.md` 를 Phase 0-6 전체 PM 으로 확장. `skills/lstack/SKILL.md` 는 orchestrator spawn 하는 얇은 레이어. Codex "재설계 규모" 지적은 시도 후 판단.
- **D4 원안 유지**: `skills/call-codex-cli/` → `skills/call-as-codex/` rename.
- **D5 원안 유지**: `agents/test-planner.md` → `agents/test-designer.md` rename.
- **수용**: approval contract SSOT 고정 · `plugin.json` + `marketplace.json` 버전 동기 · CHANGELOG 에 old→new 매핑 · rename AC 에 smoke 3종 포함.
- **실행 순서**: approval SSOT → rename 병렬 → orchestrator PM 승격 → 2.0.1 bump.

### T9: approval contract SSOT 정리 (exec: oh-my-claudecode:executor) — 완료 `c134e14`
수정: `docs/spec/ARCHITECTURE.md` — `## plan.md 섹션 → Phase 매핑` 표를 approval contract SSOT 로 명시. `### 최종 확정` 블록의 작성자 · 승인 방식 · Phase 2.3 → 2.4 전이 규칙을 여기서만 정의.
수정: `skills/lstack/SKILL.md` — approval/phase 관련 서술을 ARCHITECTURE 참조로 축약.
수정: `skills/write-plan-md/SKILL.md` — 섹션 소유자 표 중 `### 최종 확정` 작성자를 ARCHITECTURE SSOT 와 일치.

ARCHITECTURE.md 표 하단에 Approval contract (SSOT) 단락 추가. lstack SKILL.md Phase 0.3 은 중복 bullet 제거 후 ARCHITECTURE 참조로 축약.

- [x] AC9-1: `docs/spec/ARCHITECTURE.md` 에 `### 최종 확정` 블록의 작성자/승인 방식이 단일 위치에 명시됨 (v: verifier)
- [x] AC9-2: `skills/lstack/SKILL.md` 와 `skills/write-plan-md/SKILL.md` 내 approval 관련 서술이 ARCHITECTURE 와 상충하지 않음 (v: code-reviewer)
- [x] AC9-3: ARCHITECTURE phase 매핑 표에 `### 최종 확정` 상태 행이 SSOT 지표로 표기됨 (v: verifier)

### T10: call-codex-cli → call-as-codex rename (exec: oh-my-claudecode:executor) — 완료 `25dbac0`
수정: `git mv skills/call-codex-cli skills/call-as-codex` + SKILL.md `name` / 표기 / 예시 일괄 치환.
수정: 호출자 참조 치환 — `agents/orchestrator.md`, `commands/ask-cto.md`, `docs/spec/PRINCIPLE.md`, `docs/spec/ARCHITECTURE.md`, `skills/lstack/SKILL.md`, `skills/write-plan-md/SKILL.md`, `agents/planner.md`, `CHANGELOG.md`.
제외: `docs/worklogs/*` (역사적 기록 보존).

10개 파일, `replace_all` 로 일괄 치환. CHANGELOG.md 도 현행 상태 반영을 위해 갱신.

- [x] AC10-1: `rg 'call-codex-cli'` 결과 `docs/worklogs/` 제외 0 건 (v: verifier)
- [x] AC10-2: `skills/call-as-codex/SKILL.md` 존재, `name: call-as-codex`, 표기 예시 새 이름 (v: verifier)
- [x] AC10-3: smoke 3종 — (a) Phase 2.1 설계 호출 지점 (lstack), (b) Phase 3+4 review fan-out (orchestrator), (c) Phase verdict (orchestrator) 모두 `call-as-codex` 표기 (v: code-reviewer)

### T11: test-planner → test-designer rename (exec: oh-my-claudecode:executor) — 완료 `306c6e6`
수정: `git mv agents/test-planner.md agents/test-designer.md` + frontmatter `name: test-designer`. description 첫 문장 ("테스트 코드를 쓰지 않는다") 유지.
수정: 호출자 참조 치환 — `agents/planner.md` / `docs/spec/ARCHITECTURE.md` / `skills/lstack/SKILL.md` / `skills/write-plan-md/SKILL.md` / `CHANGELOG.md`.
제외: `docs/worklogs/*`.

6개 파일, `replace_all` 로 일괄 치환. Role 자기 참조도 Test-Planner → Test-Designer 로 갱신.

- [x] AC11-1: `rg 'test-planner'` 결과 `docs/worklogs/` 제외 0 건 (v: verifier)
- [x] AC11-2: `agents/test-designer.md` 존재, frontmatter `name: test-designer`, "테스트 코드를 쓰지 않는다" 유지 (v: verifier)
- [x] AC11-3: smoke 정적 점검 — Phase 2.5 dispatch 지점 및 섹션 소유자 표 모두 `test-designer` (v: code-reviewer)

### T12: orchestrator Phase 0-6 PM 승격 + lstack 얇게 (exec: oh-my-claudecode:executor) (depends on: T9, T10, T11) — 완료 `0feb68d`
수정: `agents/orchestrator.md` — Role/Responsibilities 를 Phase 0-6 전체 PM 으로 확장. 각 Phase 책임 3-5 줄로 "언제 · 무엇을 dispatch":
- Phase 0: State Detect (worklog scan + plan.md 섹션 → phase 추론, ARCHITECTURE 매핑 표 참조)
- Phase 1: Interview dispatch (`hoyeon:interviewer`)
- Phase 2: 이중검토 중재 (`call-as-codex(lstack:principal-engineer) mode:design` → `mode:critique` → user 승인 → planner → test-designer)
- Phase 3+4: 현재 execution 구조 유지 (wave / fan-out / judge / ralph / rescue)
- Phase 5: Spec 업데이트
- Phase 6: Compound 호출
세부 절차는 대상 agent/prompt 에 위임.
수정: `skills/lstack/SKILL.md` — 얇은 entry 로 축소. `Agent({subagent_type: "lstack:orchestrator", name: "orchestrator", run_in_background: true, prompt: ...})` 한 번만 수행.

orchestrator.md: 302행 → 270행 (Phase 0-6 전체 PM). model inherit → opus. lstack SKILL.md: 240행 → 33행 (thin entry).

- [x] AC12-1: `agents/orchestrator.md` Role/Responsibilities 가 Phase 0-6 을 모두 커버 (v: verifier)
- [x] AC12-2: `skills/lstack/SKILL.md` 가 orchestrator dispatch 외의 phase 로직을 포함하지 않음 (v: code-reviewer)
- [x] AC12-3: smoke 3종 — (a) 새 작업 시작 경로, (b) resume 경로, (c) Phase 2 critique→approval→planner 경로 가 orchestrator.md 본문에서 정적 식별 가능 (v: code-reviewer)

### T13: 버전 2.0.1 + CHANGELOG (old→new 매핑) (exec: oh-my-claudecode:executor) (depends on: T9, T10, T11, T12) — 완료 `54eb476`
수정: `.claude-plugin/plugin.json` `"2.0.0"` → `"2.0.1"`.
수정: `.claude-plugin/marketplace.json` `"1.0.0"` → `"2.0.1"` (version SSOT drift 해소).
수정: `CHANGELOG.md` 에 `## 2.0.1 - 2026-04-15` 섹션 추가. Changed / Renamed / Fixed 분류. Renamed 에 `call-codex-cli → call-as-codex`, `test-planner → test-designer` 매핑 명시 (과거 worklog 복붙 시 참고용).

- [x] AC13-1: `plugin.json` 과 `marketplace.json` 모두 `"version": "2.0.1"` (v: verifier)
- [x] AC13-2: `CHANGELOG.md` 에 `## 2.0.1` 섹션 존재, `Renamed` 하위에 두 rename 매핑 명시 (v: verifier)
- [x] AC13-3: `Changed` 항목에 orchestrator Phase 0-6 PM 승격 + lstack 얇은 entry 축소 언급 (v: code-reviewer)


## 향후 과제

**Phase 5 spec sync 로 해결됨 (commit `ea4474e`)**:
- ~~ARCHITECTURE.md Persona source files 섹션 정리~~
- ~~ARCHITECTURE.md planner 설명 `tasks.json` → `plan.md`~~
- ~~ARCHITECTURE.md Codex Integration Pool LOC>50 게이트 제거~~
- ~~skills/lstack/SKILL.md FF review / adversarial 문구 업데이트~~

**2.0.1 확장 사이클로 승격 (이 worklog 연장으로 실행)**:
- ~~D1: orchestrator Phase 0-6 PM 승격~~ → T12
- ~~D4: `call-codex-cli` → `call-as-codex` rename~~ → T10
- ~~D5: `test-planner` → `test-designer` rename~~ → T11
