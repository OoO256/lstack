...
cat <<'EOF'
[nobs] 응답 규칙: 결론 먼저(두괄식) · 분석이면 현(상황/문제)/원(원인)/대(대응)/사(사후관리·평가) 섹션 · 신규 용어 만들지 말고 사용자가 아는 말로 · 최대한 짧게. 부차적 내용은 참조형 각주로 — 본문엔 [1] 마커만, 본문 뒤에 [1] 내용. anchor 없는 첨언은 마커 없이 - bullet.
EOF
exit 0
