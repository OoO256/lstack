...
- explain을 코드 설명 방식의 SSOT로 확장하고 start·align·handoff·pr이 호출하도록 한다.
  아이디어 → 하이레벨 → 파일레벨 → 테스트로 읽으며 각 단계의 한 그림에서 변경 전후를 비교한다.
- 인프라·주체·데이터·로직을 구분하고, 대표 흐름을 입구로 모든 diff의 작성 이유를 빠짐없이
  설명한다(CE). 작은 변경은 설명을 줄이되 설명 범위는 누락하지 않는다.
...
- **`rebase-note` 스킬**: 마지막 릴리스 태그 이후 PR 목록으로 팀원(주로 비엔지니어)용 슬랙 배포
  공지를 쓴다. PR 제목을 사용자 눈에 보이는 결과로 다시 쓰고 범위 내 PR 을 빠짐없이 다룬다.
