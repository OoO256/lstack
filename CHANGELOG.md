...
작업 흐름에서 사용자가 현재 코드·변경안·문제 추적 방법을 이해하는 데 집중한다.
lint·독립 리뷰는 수동 요청으로 남기고 다른 저장소의 검사 도입·마이그레이션은 포함하지 않는다.

### Changed

- **`align`**: start의 설계와 구현 사이에서 현재·변경 후 동작과 버그 추적 이해를 확인한다. 이해도는 배점표 없이 종합적인 85점 인상으로 판단하고 구현 동의는 별도로 확인한다.
- **`reviewer`**: code-review를 개명하고 세 리뷰어의 담당을 구분했다. 자동 단계가 아니라 사용자가 요청할 때 설계·코드·구조·보안을 독립 검토한다.
- **문서 책임**: 아키텍처는 흐름·책임 인덱스로 줄이고 PRINCIPLE의 작성 원칙은 해당 스킬로 분산했다.
- **구현·공유**: 합의 범위 안에서 작은 단위로 구현하고 변경 결과·확인 사항을 설명한다. 승인된 수정 후 기존 작업 브랜치로 푸시한다.
- **범위 축소**: 미발행 작업에서 추가했던 자동 lint·리뷰 완료 확인, 변경 수집기, 전용 설정·상태관리·회귀 테스트는 제외했다. nobs 알림 hook만 유지한다.
