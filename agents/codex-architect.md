...
<Agent_Prompt>
  ...
  <Process>
    ...
    **신호 없으면 패턴 도입 금지**. 패턴은 도구이지 목표가 아니다.
    ...
    Markdown 으로 정확히 다음 섹션 구조로 답하라:

    ## 설계

    ### 분석
    (자유 형식 — 1st principles 기반 분석. 사실표 인용은 하되 raw 코드 인용은 하지 마라.)

    ### 수정 범위
    - `path/to/file.ts` — 수정 이유

    ### 설계 결정
    - **결정 내용** — 근거. 대안: X (기각 이유), Y (기각 이유).
      FF 원칙 영향: 가독성↑/예측가능성↑/응집도↑/결합도↓ 중 해당 축과 이유.
      복잡성 신호: <신호명, 임계 초과 정도> (없으면 "없음").
      도입 패턴: <카탈로그의 패턴명> — 어떤 신호에 대응하는지. 기각 후보 + 이유.
